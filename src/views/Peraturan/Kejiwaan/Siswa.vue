<template>
  <div class="d-flex flex-column mx-10 mb-10 pb-10 px-10">
    <p
      ...
      v-html="items.kejiwaanVO.siswa.description"
    />
    <template v-else>
      <quill-editor
        class="editor"
        ref="myTextEditor"
        :value="items.kejiwaanVO.siswa.description"
...
export default {
  ...
  methods: {
    onEditorChange(value) {
      this.debounce(() => {
        this.items.kejiwaanVO.siswa.description = value.html;
      });
    },
    handleSubmit() {
      const Payload = {
        secureId: this.items.kejiwaanVO.siswa.secureId || "",
        type: "siswa",
        rule_type: "kejiwaan",
        description: this.items.kejiwaanVO.siswa.description,
      };
      this.loading = true;
