<template>
  <div class="d-flex flex-column mx-10 mb-10 pb-10 px-10">
    <p
      ...
      v-html="items.kepribadianVO.siswa.description"
    />
    <template v-else>
      <quill-editor
        class="editor"
        ref="myTextEditor"
        :value="items.kepribadianVO.siswa.description"
...
export default {
  ...
  methods: {
    onEditorChange(value) {
      this.debounce(() => {
        this.items.kepribadianVO.siswa.description = value.html;
      });
    },
    handleSubmit() {
      const Payload = {
        secureId: this.items.kepribadianVO.siswa.secureId || "",
        type: "siswa",
        rule_type: "kepribadian",
        description: this.items.kepribadianVO.siswa.description,
      };
      this.loading = true;
