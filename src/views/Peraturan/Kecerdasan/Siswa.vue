<template>
  <div class="d-flex flex-column mx-10 mb-10 pb-10 px-10">
    <p
      ...
      v-html="items.kecerdasanVO.siswa.description"
    />
    <template v-else>
      <quill-editor
        class="editor"
        ref="myTextEditor"
        :value="items.kecerdasanVO.siswa.description"
...
export default {
  ...
  methods: {
    onEditorChange(value) {
      this.debounce(() => {
        this.items.kecerdasanVO.siswa.description = value.html;
      });
    },
    handleSubmit() {
      const Payload = {
        secureId: this.items.kecerdasanVO.siswa.secureId || "",
        type: "siswa",
        rule_type: "kecerdasan",
        description: this.items.kecerdasanVO.siswa.description,
      };
      this.loading = true;
