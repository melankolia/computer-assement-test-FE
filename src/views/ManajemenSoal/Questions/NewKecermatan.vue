...
<script>
import QuestionService from "@/services/resources/Questions/new-kecermatan.service";
  ...
  methods: {
    getDetail() {
      ...
      QuestionService.getDetail({
        secureId: this.newKecermatanSecureId || this.id,
        type: "admin",
      })
    ...
    handleSaveSection(i) {
      const Payload = {
        groupSecureId: this.newKecermatanSecureId || this.id,
    ...
    deleteData() {
      ...
      GroupService.deleteNewKecermatan(this.newKecermatanSecureId || this.id)
        .then(({ data: { result, message } }) => {
          if (message == "OK") {
            this.$store.commit("snackbar/setSnack", {
              show: true,
        ...
        .catch((err) => {
          ...
          this.$store.commit("snackbar/setSnack", {
            show: true,
            message: "Gagal menghapus data New Kecermatan",
            color: "error",
          });
        });
    },
